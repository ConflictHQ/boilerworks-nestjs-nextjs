...
builder.queryField("permissions", (t) =>
  t.prismaField({
    type: ["Permission"],
...
builder.mutationField("createGroup", (t) =>
  t.field({
    ...
    resolve: async (_root, args, ctx) => {
      ...
      if (existing) return mutationError("name", "Group already exists");
...
builder.mutationField("assignUserToGroup", (t) =>
  t.field({
    type: MutationResult,
    args: {
...
builder.mutationField("removeUserFromGroup", (t) =>
  t.field({
    type: MutationResult,
    args: {
      userId: t.arg.string({ required: true }),
...
builder.mutationField("assignPermissionToGroup", (t) =>
  t.field({
    type: MutationResult,
    args: {
      groupId: t.arg.string({ required: true }),
...
builder.mutationField("removePermissionFromGroup", (t) =>
  t.field({
    type: MutationResult,
    args: {
      groupId: t.arg.string({ required: true }),
